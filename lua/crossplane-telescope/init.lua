    ...
    on_exit = function(job_id, exit_status, event_type)
      if exit_status ~= 0 then
        ...
        vim.defer_fn(function()
          vim.api.nvim_buf_set_lines(term_bufnr, 0, 0, false, {"Error: kubectl command failed. See output for details."})
        end, 500)
      else
  ...
  pickers.new({}, {
    ...
    previewer = previewers.new_buffer_previewer({
      ...
      define_preview = function(self, entry, status)
        ...
        local resource_details = vim.fn.system(string.format('kubectl get %s -o yaml', entry.value))

        if vim.v.shell_error ~= 0 then
          vim.api.nvim_buf_set_lines(self.state.bufnr, 0, -1, false, { "Failed to get resource details." })
        else
